...
test_config(void)
{
        struct fpc_pid_config cfg = {.kp = 1000,
                                     .ki = 500,
                                     .kd = 200,
                                     .dt = 1000,
                                     .out_min = -100,
...
TEST_CASE(test_saturation_reporting)
{
        ...
        TEST_ASSERT(fpc_pid_pool_init() == FPC_STATUS_OK);
        TEST_ASSERT(fpc_pid_init(&ctx, &cfg) == FPC_STATUS_OK);
        TEST_ASSERT(fpc_pid_compute(ctx, 10000, 0, &output)
...
TEST_CASE(test_reset_clears_runtime_state)
{
        ...
        TEST_ASSERT(fpc_pid_pool_init() == FPC_STATUS_OK);
        TEST_ASSERT(fpc_pid_init(&ctx, &cfg) == FPC_STATUS_OK);
        TEST_ASSERT(fpc_pid_compute(ctx, 1000, 0, &output)
                    == FPC_STATUS_SATURATED);
...
TEST_CASE(test_set_config_clamps_state)
{
        ...
        TEST_ASSERT(fpc_pid_compute(ctx, 1000, 0, &output)
                    == FPC_STATUS_SATURATED);
        TEST_ASSERT(fpc_pid_set_config(ctx, &tighter_cfg)
                    == FPC_STATUS_SATURATED);
        TEST_ASSERT(fpc_pid_get_state(ctx, &state) == FPC_STATUS_OK);
...
TEST_CASE(test_manual_mode_and_bumpless_return)
{
        ...
        TEST_ASSERT(fpc_pid_pool_init() == FPC_STATUS_OK);
        TEST_ASSERT(fpc_pid_init(&ctx, &cfg) == FPC_STATUS_OK);
        TEST_ASSERT(fpc_pid_set_mode(ctx, FPC_PID_MODE_MANUAL, 300)
                    == FPC_STATUS_OK);
        ...
        TEST_ASSERT(fpc_pid_set_mode(ctx, FPC_PID_MODE_AUTO, 0)
                    == FPC_STATUS_OK);
        TEST_ASSERT(fpc_pid_get_state(ctx, &state) == FPC_STATUS_OK);
        TEST_ASSERT(state.mode == FPC_PID_MODE_AUTO);
...
TEST_CASE(test_manual_mode_clamps_output)
{
        ...
        TEST_ASSERT(fpc_pid_pool_init() == FPC_STATUS_OK);
        TEST_ASSERT(fpc_pid_init(&ctx, &cfg) == FPC_STATUS_OK);
        TEST_ASSERT(fpc_pid_set_mode(ctx, FPC_PID_MODE_MANUAL, 1000)
                    == FPC_STATUS_SATURATED);
...
TEST_CASE(test_overflow_detection)
{
        ...
        TEST_ASSERT(fpc_pid_pool_init() == FPC_STATUS_OK);
        TEST_ASSERT(fpc_pid_init(&ctx, &cfg) == FPC_STATUS_OK);
        TEST_ASSERT(fpc_pid_compute(ctx, INT32_MAX, INT32_MIN, &output)
                    == FPC_STATUS_OVERFLOW);
...
run_pid_tests(void)
{
        ...
        run_test(test_manual_mode_and_bumpless_return,
                 "manual_mode_and_bumpless_return");
        run_test(test_manual_mode_clamps_output, "manual_mode_clamps_output");
        run_test(test_null_and_invalid_runtime_calls,
                 "null_and_invalid_runtime_calls");
